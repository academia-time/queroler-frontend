...
import {
  AditionalDataRequestDTO,
  aditionalDataSchema,
} from '@/core/application/user/aditional-data.dto';

export async function updateWithAditionalDataAction(
  data: AditionalDataRequestDTO
) {
  const validated = aditionalDataSchema.safeParse(data);
